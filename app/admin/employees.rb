...
ActiveAdmin.register Employee do
  ...
  form do |f|
    ...
    f.inputs do
      f.input :restaurant_id, as: :select, collection: Restaurant.all.collect { |restaurant|
                                                         [restaurant.name, restaurant.id]
                                                       }
      f.input :manager_id, as: :select, collection: Employee.where(designation: 1).collect { |manager|
                                                      [manager.first_name, manager.id]
                                                    }
    end
    f.inputs :email
